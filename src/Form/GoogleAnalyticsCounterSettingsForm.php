...
class GoogleAnalyticsCounterSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $default_start_dates = (array) json_decode($config->get('general_settings.start_date'));
  ...
  {
    //TODO: Ensure that atleast one "Start Date" is selected.
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($values['start_date'] as $key => $value) {
      if($value === 0){
        unset($values['start_date'][$key]);
      }
    }
    ...
    $config
      ...
      ->set('general_settings.start_date',  json_encode($values['start_date']))
